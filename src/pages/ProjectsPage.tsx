    ...
    <div className="space-y-6">
      <DataTable
        ...
        columns={[
          ...
          { header: 'Status', render: (project) => <StatusBadge status={project.status} /> },
        ]}
      />

      <section className="grid gap-4 xl:grid-cols-4">
        {(['Proposta', 'Em andamento', 'Revisão', 'Finalizado'] as const).map((status) => (
          <article key={status} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-100">
              ...
              {projects
                ...
                .map((project) => (
                  <div key={project.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <p className="mt-3 text-xs font-medium uppercase tracking-[0.15em] text-slate-400">Prazo {project.deadline}</p>
                  </div>
                ))}
            </div>
          </article>
