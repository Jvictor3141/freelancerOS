...
export const paymentStatusFilterOptions = ['all', ...paymentStatuses] as const;
...
/**
 * Maps each payment status to its i18n translation key.
 * Usage: t(paymentStatusLabel[status])
 */
export const paymentStatusLabel: Record<PaymentStatus, string> = {
  pending: 'status.payment.pending',
  paid: 'status.payment.paid',
  overdue: 'status.payment.overdue',
};
