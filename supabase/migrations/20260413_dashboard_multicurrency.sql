-- Rebuild the payments read model to expose the currency column.
-- This was omitted in the original view, causing all downstream aggregations
-- to mix BRL/USD/EUR amounts as if they were the same denomination.
...
-- Rebuild the dashboard snapshot function.
-- Key changes:
--   • paymentMetrics is now an array of rows grouped by currency instead of
--     a single merged object — cross-currency summation was factually wrong.
--   • revenue is now grouped by (month, currency) — same issue.
--   • recentActivities now includes project.currency.
--   • paymentAlerts now includes payment.currency.
...
-- Revenue is returned as sparse (month, currency, amount) tuples.
-- The frontend is responsible for filling the 6-month window and zero-padding
-- missing (month, currency) combinations — this avoids the NULL-coalesce bug
-- from the previous generate_series left join approach.
revenue as (
  select
...
-- Rebuild the client details snapshot to include currency in projects/payments.
