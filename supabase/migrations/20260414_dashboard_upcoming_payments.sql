-- Add upcomingPayments to the dashboard snapshot.
-- Separate from paymentAlerts (overdue/pending attention) — this query
-- targets only pending payments due in the next 30 days, so the
-- DashboardUpcomingDueDates component always receives the right data
-- regardless of how many overdue items exist.
...
upcoming_payments as (
  ...
  where payment.user_id = auth.uid()
    ...
    and payment.due_date >= timezone('America/Sao_Paulo', now())::date
