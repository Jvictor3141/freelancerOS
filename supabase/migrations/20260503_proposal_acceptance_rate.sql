-- Replace average_ticket (cross-currency average, semantically invalid) with
-- proposal_acceptance_rate: accepted / total-sent proposals × 100.
-- "Sent" means any proposal that left draft (sent, accepted, rejected).
        ...
        select
          case
            when count(*) filter (where status != 'draft') = 0 then 0
              ...
              / count(*) filter (where status != 'draft') * 100,
              1
            )
          end
